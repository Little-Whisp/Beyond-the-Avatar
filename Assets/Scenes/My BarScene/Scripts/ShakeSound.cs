...
public class ShakeSound : MonoBehaviour
{
    ...
    [Header("Audio Loop")]
    public float silenceToStop = 0.25f;
    public float fadeOutTime = 0.10f;
    ...
    void Update()
    {
        ...
        // ✅ NEW: must have liquid to play sound
        if (!shaker || !shaker.HasLiquid)
        {
            ResetFrame();
            StopLoop(false);
            return;
        ...
        if (isShake)
        {
            ...
            {
                shaker.AddMixEnergy(mixGainPerShake);
